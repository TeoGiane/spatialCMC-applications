# # ---- POISSON MODEL ON LATTICE - GENERATE PLOTS ---- # #
...
mun_sf$best_clust <- as.factor(salso::salso(cluster_allocs, loss = "VI"))
...
pdf(file.path(out_dir, "plt_psm.pdf"), height = 4, width = 4); print(plt_psm); dev.off()

# Plot - True cluster on the geometry
plt_true_clust <- ggplot() +
  geom_sf(data = mun_sf, aes(fill=true_clust), color='gray25', linewidth=0.5, alpha=0.75) +
...
if(run_cmc){
  plt_true_clust <- plt_true_clust +
    geom_sf(data = geom_prov, color='darkred', fill=NA, linewidth=2)
}
pdf(file.path(out_dir, "plt_true_clust.pdf"), height = 4, width = 4); print(plt_true_clust); dev.off()

# Plot - Best cluster on the geometry
plt_best_clust <- ggplot() +
  geom_sf(data = mun_sf, aes(fill=best_clust), color='gray25', linewidth=0.5, alpha=0.75) +
  scale_fill_manual(values = c("1" = "steelblue", "2" = "darkorange")) +
  guides(fill = guide_legend(title = "Cluster", title.position = "bottom", title.hjust=0.5,
                             label.position = "bottom")) +
  theme_void() + theme(legend.position = "bottom")
